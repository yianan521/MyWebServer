...
WebServer::WebServer()
{
    ...
    strcat(m_root, root);
    // 添加连接池参数
    m_http_conn_pool_size = 100;
    m_http_conn_timeout = 15;
    m_http_pool = NULL;
    printf("DEBUG: 设置 m_http_pool = NULL 完成\n");
    printf("DEBUG: 设置 m_http_pool = NULL 完成\n");
    printf("DEBUG: WebServer构造函数完成\n");
    LOG_INFO("WebServer构造函数：设置默认连接池参数");
    //定时器
...
// 添加初始化连接池的方法
void WebServer::http_conn_pool_init() {
    ...
    printf("DEBUG: 大小=%d, 超时=%d\n", m_http_conn_pool_size, m_http_conn_timeout);
    LOG_INFO("开始初始化HTTP连接池，大小：%d，超时：%d秒", 
             m_http_conn_pool_size, m_http_conn_timeout);
    m_http_pool = http_conn_pool::get_instance();
    printf("DEBUG: get_instance() 返回: %p\n", m_http_pool);
    printf("DEBUG: get_instance() 返回: %p\n", m_http_pool);
    m_http_pool->init(m_http_conn_pool_size, m_http_conn_timeout);
    printf("DEBUG: 调用 init 前\n");
    printf("DEBUG: 调用 init 前\n");
    
    Log::get_instance()->write_log(0, "HTTP连接池初始化完成，大小：%d，超时：%d秒", 
                                  m_http_conn_pool_size, m_http_conn_timeout);
}
    ...
    while (!stop_server) {
        ...
        for (int i = 0; i < number; i++) {
            ...
            if (sockfd == m_listenfd) {
                ...
                // 使用连接池获取连接对象
                http_conn* conn = m_http_pool->get_conn();
                if (!conn) {
                    Log::get_instance()->write_log(1, "获取HTTP连接失败");
                    continue;
                }
                
                // 设置连接socket
                conn->init(sockfd, (users_timer[sockfd].address), m_root, m_CONNTrigmode, m_close_log, m_user, m_passWord, m_databaseName);
                conn->update_activity_time();
                
                dealwithread(sockfd);
            } else if (events[i].events & EPOLLOUT) {
                // 处理写事件
                http_conn* conn = users + sockfd;  // 临时使用原有方式
